with open('staff/views.py', 'r', encoding='utf-8') as f:
    content = f.read()

# Add a decorator that excludes managers
new_decorator = '''
def staff_required(view_func):
    def wrapper(request, *args, **kwargs):
        if not request.user.is_authenticated:
            return redirect('/')
        
        # Check if user is a manager - redirect to manager portal
        try:
            from main.models import StaffProfile
            profile = StaffProfile.objects.get(user=request.user)
            if profile.position.lower() == 'manager':
                return redirect('/manager/dashboard/')
        except:
            pass
        
        if request.user.is_superuser:
            return view_func(request, *args, **kwargs)
        if request.user.is_staff:
            return view_func(request, *args, **kwargs)
        
        messages.error(request, 'Access denied. Staff access required.')
        return redirect('/')
    return wrapper
'''

# Find and replace the staff_required decorator
if 'def staff_required' in content:
    import re
    pattern = r'def staff_required\(view_func\).*?return wrapper'
    content = re.sub(pattern, new_decorator, content, flags=re.DOTALL)
    with open('staff/views.py', 'w', encoding='utf-8') as f:
        f.write(content)
    print('✓ Updated staff_required decorator - excludes managers')
